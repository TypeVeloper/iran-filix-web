...
const HeroNewMovie: React.FC<Props> = ({
  ...
  const [currentSlide, setCurrentSlide] = React.useState<MoviesType>(movies[0]);
  ...
  return (
    <div
      className={` hero-slide w-full h-[500px] flex items-center justify-center relative overflow-hidden rounded-xl  ${className} `}>
      <ProgressBar level={countTime} className='absolute bottom-0 ' />
      <>
        <div
          className='w-full h-full overflow-hidden !bg-cover !bg-top !blur-md !bg-no-repeat'
          style={{
            backgroundImage: ` linear-gradient(190deg, #000000, #0000) , url(${currentSlide?.cover})`,
          }}></div>
      ...
      <div className='w-[50%] h-full absolute bottom-0 right-0 '>
        <Carousel
          ...
          onActiveIndexChange={({ realIndex }: any) => {
            setCurrentSlide(movies[realIndex]);
          }}
          centeredSlides={true}>
          {movies.length &&
